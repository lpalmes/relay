...
[
  {
    name: 'Object',
    impl: RelayModernRecordObjImpl,
  },
  {
    name: 'Map',
    impl: RelayModernRecordMapImpl,
  },
  {
    name: 'Reason',
    impl: RelayModernRecordReason,
  },
].forEach(Record => {
  describe('RelayModernRecord with ' + Record.name, () => {
    const RelayModernRecord = Record.impl;
    ...
    describe('setLinkedRecordID()', () => {
      it('sets a link', () => {
        const record = RelayModernRecord.create('4', 'User');
        RelayModernRecord.setLinkedRecordID(record, 'pet', 'beast');
        expect(RelayModernRecord.getLinkedRecordID(record, 'pet')).toBe(
          'beast',
        );
      });
    });
    ...
    describe('getValue()', () => {
      ...
      it('returns null', () => {
        expect(
          RelayModernRecord.getValue(record, 'blockbusterMembership'),
        ).toBe(null);
      });
    ...
    describe('setValue()', () => {
      it('warns if updating to a different __id', () => {
        ...
        const record = RelayModernRecord.create('4', 'User');
        expect(() =>
          RelayModernRecord.setValue(record, ID_KEY, 'not-4'),
        ).toWarn([
