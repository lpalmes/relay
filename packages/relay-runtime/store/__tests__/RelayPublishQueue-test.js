...
describe('RelayPublishQueue', () => {
  ...
  describe('applyUpdate()/revertUpdate()', () => {
    ...
    beforeEach(() => {
      source = RelayRecordSource.fromJSON(simpleClone(initialData));
  ...
  describe('commitPayload()', () => {
    it('publishes the source to the store', () => {
      ...
      const queue = new RelayPublishQueue(store, null, defaultGetDataID);
      const publishSource = RelayRecordSource.create();
    ...
    it('processes handle fields before publishing', () => {
      const notify = jest.fn();
      const publish = jest.fn();
      const source = RelayRecordSource.fromJSON();
      const store = {
        getSource: () => source,
        notify,
        publish,
    ...
    it('applies optimistic updates and commits server data together', () => {
      ...
      const source = RelayRecordSource.fromJSON(simpleClone(initialData));
    ...
    it('can rollback an optimistic mutation after committing an updater', () => {
      ...
      const getVolume = () =>
        RelayModernRecord.toObj(store.getSource().get('84872')).volume;
  ...
  describe('commitSource()', () => {
    it('publishes the source to the store', () => {
      const notify = jest.fn();
      const publish = jest.fn();
      const store_source = RelayRecordSource.fromJSON();
      const store = {
    ...
    describe('it commits in order', () => {
      ...
      beforeEach(() => {
        ...
        nameSource = RelayRecordSource.fromJSON({
          4: {
            [ID_KEY]: '4',
            [TYPENAME_KEY]: 'User',
            id: '4',
